...
def confidence_interval_for_sigma_unknown_mu(random_sample, alpha):
    mean = np.mean(random_sample)
    n = random_sample.shape[0]
    sigma = np.sum(np.power(random_sample - mean, 2)) / (n - 1)
...
def plt3_confidence_intervals(title, mus, sigmas, random_sample_gens, alpha, total_size=50):
    ...
    for plot_num, (random_sample_gen, mu, sigma) in enumerate(zip(random_sample_gens, mus, sigmas)):
        ...
        for i in range(total_size):
            ...
            mean = np.mean(random_sample)
            lower_bound, upper_bound, size = confidence_interval_for_sigma_unknown_mu(random_sample, alpha)
            n = random_sample.shape[0]
            sample_sigma = np.sum(np.power(random_sample - mean, 2)) / (n - 1)
